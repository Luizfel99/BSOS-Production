...
export default function GestaoAirbnb() {
  ...
  const [activeTab, setActiveTab] = useState<
    "propriedades" | "reservas" | "calendario"
  >("propriedades");
  ...
  const [propriedades, setPropriedades] = useState<Propriedade[]>([
    {
      ...
      instrucoes:
        "Apartamento no 5º andar. Elevador funcionando. Chave na portaria com Sr. João.",
      tempoLimpeza: 120,
      valor: 80,
      airbnb_id: "ABB123456",
      hostaway_id: "HOST789",
    ...
    },
    {
      ...
      instrucoes:
        "Casa térrea. Chaves na caixa ao lado do portão. Cuidado com o cachorro dos vizinhos.",
      tempoLimpeza: 180,
      valor: 120,
      airbnb_id: "ABB789012",
      status: "ativo",
  ...
  const getPropriedadeNome = (propriedadeId: string) => {
    const prop = safeArray.find(
      propriedades,
      (p) => safeGet.string(p?.id) === propriedadeId,
    );
    return prop
      ? safeGet.string(prop.nome, "Propriedade")
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          ...
          <p className="text-2xl font-bold text-blue-600">
            R${" "}
            {reservas
              .reduce((acc, r) => acc + r.valor_limpeza, 0)
              .toLocaleString()}
          </p>
        </div>
      </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNovaPropriedade({
                      ...novaPropriedade,
                      tempoLimpeza: parseInt(e.target.value),
                    })
                  }
                  className="w-full border border-gray-300 rounded-lg px-3 py-2"
                  min="30"
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNovaPropriedade({
                      ...novaPropriedade,
                      chavesLocalizacao: e.target.value,
                    })
                  }
                  className="w-full border border-gray-300 rounded-lg px-3 py-2"
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                          <span>
                            {propriedade.tipo.charAt(0).toUpperCase() +
                              propriedade.tipo.slice(1)}
                          </span>
                        ...
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium ${
                            propriedade.status === "ativo"
                              ...
                              : propriedade.status === "inativo"
                                ? "bg-red-100 text-red-700"
                                : "bg-yellow-100 text-yellow-700"
                          }`}
                        >
                          {propriedade.status.charAt(0).toUpperCase() +
                            propriedade.status.slice(1)}
                        </span>
                        {propriedade.airbnb_id && (
                          <span className="text-xs text-gray-500">
                            Airbnb: {propriedade.airbnb_id}
                          </span>
                        )}
                        {propriedade.hostaway_id && (
                          <span className="text-xs text-gray-500">
                            Hostaway: {propriedade.hostaway_id}
                          </span>
                        )}
                      </div>
                    </div>
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">
                        {getPropriedadeNome(reserva.propriedade_id)}
                      </p>
                      <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                        <span>
                          Check-in:{" "}
                    ...
                    <div className="flex flex-col items-end space-y-2">
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(reserva.status)}`}
                      >
                      ...
                      <div className="flex space-x-2">
                        ...
                        {reserva.status === "limpeza_agendada" && (
                          <button
                            onClick={() =>
                              atualizarStatusReserva(reserva.id, "pronto")
                            }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            {["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"].map((dia) => (
              <div
                key={dia}
                className="p-2 text-center font-medium text-gray-700 bg-gray-50"
              >
                {dia}
              </div>
            ))}
            ...
            {Array.from({ length: 31 }, (_, i) => i + 1).map((dia) => {
              const reservasDoDia = reservas.filter(
                (r) =>
                  new Date(r.checkout).getDate() === dia &&
                  r.status === "limpeza_agendada",
              );
              ...
              return (
                <div key={dia} className="p-2 border border-gray-200 min-h-20">
                  ...
                  {reservasDoDia.map((reserva) => (
                    <div
                      key={reserva.id}
                      className="text-xs bg-purple-100 text-purple-700 p-1 rounded mt-1"
                    >
                      {reserva.guest_name}
                    </div>
                  ))}
