...
export default function ControlePagamentos() {
  ...
  const [employeeFinancials, setEmployeeFinancials] = useState<
    EmployeeFinancial[]>(
    []);

  const [activeTab, setActiveTab] = useState<
    "payments" | "bonuses" | "employees" | "rules" | "reports">(
    "payments");
  ...
  const fetchEmployeeFinancials = async () => {
    try {
      const response = await fetch(
        `/api/payments/employees?period=${selectedPeriod}`
      );
      ...
      console.error(
        "Erro ao buscar dados financeiros dos funcionários:",
        error
      );
  ...
  const calculateAutomaticBonus = (payment: Payment) => {
    ...
    bonusRules.forEach((rule) => {
      ...
      switch (rule.trigger.metric) {
        case "rating":
          qualifies =
          (payment.details?.averageRating ?? 0) >= rule.trigger.value;
          break;
        case "cleanings":
          qualifies =
          (payment.details?.cleaningsCompleted ?? 0) >= rule.trigger.value;
          break;
        default:
          qualifies = false;
      }
      ...
      if (qualifies) {
        if (rule.bonus.type === "fixed") {
          ...
          bonusAmount =
          (payment.details?.averageRating ?? 0) * rule.bonus.amount;
        }
  ...
  const handleExportPayments = async () => {
    try {
      const response = await fetch(
        `/api/payments/export?period=${selectedPeriod}&status=${filterStatus}&employee=${filterEmployee}`
      );
  ...
  const filteredPayments = payments.filter((payment) => {
    ...
    if (filterEmployee !== "all" && payment.employeeId !== filterEmployee)
    return false;
    if (
    searchQuery &&
  ...
  const totalBonusesGiven = payments.reduce(
    (acc, p) => acc + p.performanceBonus + p.qualityBonus + p.punctualityBonus,
    0
  );
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Gestão automática de pagamentos e bonificações
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-gradient-to-r from-purple-500 to-purple-600 p-4 rounded-lg text-white">
          <div className="text-2xl font-bold">
            R$ {totalBonusesGiven.toLocaleString()}
          </div>
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                className="w-12 h-12 rounded-full" />
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      {payment.propertyName} • {payment.cleaningDate}
                    </p>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm">
                          {payment.details?.averageRating?.toFixed(1) ?? "N/A"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="text-right">
                  <div
                className={`inline-flex items-center gap-2 px-3 py-1 rounded-full font-medium ${getStatusColor(payment.status)}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-center">
                  <div className="text-lg font-bold text-green-600">
                    +R${" "}
                    {(
                payment.performanceBonus +
                payment.qualityBonus +
                payment.punctualityBonus).
                toFixed(2)}
                  </div>
          ...
          <div className="mb-4 p-3 bg-blue-50 rounded-lg">
              ...
              <div className="flex gap-2">
                  ...
                  className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">
                      ...
                      <WiredButton
                  onClick={() =>
                  processPayment(payment.id, "bank_transfer")
                  }
                  className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
        ...
        <div
          ...
          className="bg-white border rounded-lg p-6">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                className="w-16 h-16 rounded-full" />
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      {employee.cleaningsCompleted} limpezas no mês
                    </p>
                    <div className="flex items-center gap-2 mt-1">
                      <Star className="h-4 w-4 text-yellow-400 fill-current" />
                      <span>
                        {employee.averageRating?.toFixed(1) ?? "N/A"} média
                      </span>
                      <span className="text-gray-400">•</span>
                      <span>
                        R${" "}
                        {employee.averageEarningPerCleaning?.toFixed(2) ??
                    "0.00"}{" "}
                        por limpeza
                      </span>
                    </div>
              ...
              <div className="mb-4">
                ...
                employee.goalProgress >= 100 ?
                "bg-green-500" :
                employee.goalProgress >= 80 ?
                "bg-blue-500" :
                "bg-orange-500"}`
                }
                style={{
                  width: `${Math.min(employee.goalProgress, 100)}%`
                }}>
              </div>
                </div>
              ...
              <div
                ...
                className="text-center p-3 bg-gray-50 rounded">
                      ...
                      <div className="text-xs text-gray-500">
                        {month.cleanings} limpezas • Rating:{" "}
                        {month.averageRating?.toFixed(1) ?? "N/A"}
                      </div>
                    </div>
              )}
                </div>
