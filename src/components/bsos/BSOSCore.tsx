...
export default function BSOSCore() {
  ...
  const handleFinalizeChecklist = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/checklists/${selectedTask.id}/complete`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleViewStatistics = async (type: string) => {
    try {
      ...
      const response = await fetch(
        `/api/statistics/checklist-templates/${type}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
          ...
          <div className="px-4 sm:px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0">
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-semibold text-gray-900 truncate">
                {getTypeIcon(selectedTask.type)} Checklist -{" "}
        ...
        <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-hidden">
          ...
          <div className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
            <p className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">
              Analisando suas tarefas e detectando oportunidades de
              otimização...
            </p>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg max-w-4xl w-full max-h-[95vh] overflow-hidden">
          ...
          <div className="p-4 sm:p-6 overflow-y-auto max-h-[calc(95vh-120px)]">
            <p className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">
              Conecte suas plataformas de reservas para sincronização automática
            ...
            <div className="mt-8 border-t pt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-gray-600 mb-3">
                    Frequência de atualização das reservas
                  </p>
        ...
        <div className="bg-white rounded-lg max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden">
          ...
          <div className="px-6 py-4 border-t border-gray-200 flex justify-between">
            <div className="flex items-center space-x-4 text-sm text-gray-600">
              ...
              <span>
                📸 {checklist.filter((item) => item.photoRequired).length} fotos
              </span>
  ...
  const tabs = allTabs.filter((tab) => {
    ...
    if (tab.allowedRoles && user && !tab.allowedRoles.includes(user.role)) {
      return false;
    }
    // Check if feature is accessible
    if (tab.requiredFeature && !canAccessFeature(tab.requiredFeature)) {
      return false;
    }
    return true;
  });
