...
interface BSOSLayoutProps {
  ...
  onModuleChange: (
  module: "core" | "manager" | "client" | "finance" | "analytics")
  => void;
}

export default function BSOSLayout({
  children,
  currentModule,
  onModuleChange
}: BSOSLayoutProps) {
  ...
  const roleModuleAccess: Record<string, string[]> = {
    ...
    client: ["client"],
    admin: ["core", "manager", "client", "finance", "analytics"]
  };
  ...
  const availableModules = modules.filter(
    (module) => user && roleModuleAccess[user.role]?.includes(module.id)
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="mobile-container">
          <div className="flex justify-between items-center h-14 sm:h-16">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3">
                  <div
                  className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium ${roleInfo.color}`}>
      ...
      {/* Content Area */}
      <main className="mobile-container py-4 sm:py-8">{children}</main>
