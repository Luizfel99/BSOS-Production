...
export class HostawayApiService {
  ...
  async getReservations(): Promise<Reservation[]> {
    try {
      ...
      return (
        data.result?.map((reservation: any) => ({
  ...
  async getAvailableCleaners(date: string, duration: number): Promise<any[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/availability?date=${date}&duration=${duration}`,
        {
          headers: {
            Authorization: `Bearer ${this.credentials.apiKey}`,
            "Content-Type": "application/json",
...
export class IntegrationOrchestrator {
  ...
  async syncAllData(): Promise<{
    ...
    try {
      ...
      if (this.hostawayService) {
        ...
        const hostawayReservations =
          await this.hostawayService.getReservations();
  ...
  async createCleaningTasksFromReservations(
    reservations: Reservation[],
  ): Promise<CleaningTask[]> {
    ...
    for (const reservation of reservations) {
      try {
        ...
        console.error(
          `Erro ao criar tarefas para reserva ${reservation.id}:`,
          error,
        );
      }
    }
  ...
  async autoAssignCleaners(tasks: CleaningTask[]): Promise<void> {
    ...
    for (const task of tasks) {
      try {
        ...
        console.error(
          `Erro ao atribuir cleaner para tarefa ${task.id}:`,
          error,
        );
      }
    }
