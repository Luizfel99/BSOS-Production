...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith("/api/")) {
    ...
    response.headers.set(
      ...
      config.app.isProduction ? config.app.url : "*",
    );
    response.headers.set(
...
function handleRouteProtection(request: NextRequest): NextResponse | null {
  ...
  const requiresAuth = PROTECTED_ROUTES.auth.some(
    (route) => pathname === route || pathname.startsWith(route + "/"),
  );
...
function validateWebhookSignature(
  ...
  const platform = new URLSearchParams(pathname.split("?")[1] || "").get(
    "platform",
  );
...
function validateWhatsAppWebhook(request: NextRequest): NextResponse | null {
  ...
  if (
    hubMode === "subscribe" &&
    hubVerifyToken === config.whatsapp.webhookVerifyToken
  ) {
