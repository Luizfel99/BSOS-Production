...
class ConfigService {
  ...
  get airbnb() {
    return {
      ...this.config.airbnb,
      isConfigured: !!(
        this.config.airbnb.clientId && this.config.airbnb.clientSecret
      ),
    };
  }
  ...
  get whatsapp() {
    return {
      ...
      isConfigured: !!(
        this.config.whatsapp.apiToken && this.config.whatsapp.phoneNumberId
      ),
    };
  }
...
if (typeof window === "undefined" && config.app.isProduction) {
  ...
  if (!validation.isValid) {
    console.warn(
      `⚠️ Missing environment variables: ${validation.missing.join(", ")}`,
    );
  }
}
