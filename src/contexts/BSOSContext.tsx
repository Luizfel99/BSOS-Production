...
export function BSOSProvider({ children }: { children: ReactNode }) {
  ...
  const updateTaskStatus = useCallback(
    async (taskId: string, newStatus: CleaningTask["status"]) => {
      try {
        // Set loading state
        ...
        const result: ApiResponse<{ task: CleaningTask }> =
          await response.json();
  ...
  const handleEditTemplate = useCallback(async (type: string) => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to edit template";
  ...
  const handleConfigureIntegration = useCallback(
    async (integrationId: string) => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to configure integration: ${response.statusText}`,
          );
        }
        ...
        if (result.success) {
          console.log(
            `✅ Integration ${integrationId} configured successfully:`,
            result.data,
          );
  ...
  return <BSOSContext.Provider value={value}>{children}</BSOSContext.Provider>;
}
