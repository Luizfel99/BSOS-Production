...
export async function GET(request: NextRequest, { params }: RouteContext) {
  let categoryParam: SettingCategory = 'general'; // default value
  ...
  try {
    ...
    categoryParam = category as SettingCategory;
    ...
    if (key) {
      const value = await getSettingValue(categoryParam, key, userRole);
      ...
      return NextResponse.json({
        success: true,
        data: { category: categoryParam, key, value },
        message: 'Setting retrieved successfully'
      });
    }

    // Get all settings for the category
    const settings = await getSettingsByCategory(categoryParam, userRole);

    return NextResponse.json({
      success: true,
      data: settings,
      message: `Settings for ${categoryParam} retrieved successfully`
    });

  } catch (error) {
    console.error(`GET Settings [${categoryParam}] error:`, error);
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const userData = JSON.parse(userCookie);
    const userRole: UserRole = userData.role || 'CLEANER';
    const categoryParam = category as SettingCategory;

    // Check category access
    if (!hasSettingAccess(userRole, categoryParam)) {
      return NextResponse.json(
        { success: false, message: `Access denied to ${categoryParam} settings` },
        { status: 403 }
      );
    }
    ...
    console.error(`POST Settings [${categoryParam}] error:`, error);
...
export async function PUT(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    console.error(`PUT Settings [${categoryParam}] error:`, error);
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const userData = JSON.parse(userCookie);
    const userRole: UserRole = userData.role || 'CLEANER';
    categoryParam = categoryValue as SettingCategory;
    ...
    await deleteSetting(categoryParam, key, userRole);

    return NextResponse.json({
      success: true,
      message: `Setting ${key} deleted from ${categoryParam} successfully`
    });

  } catch (error) {
    console.error(`DELETE Settings [${categoryParam}] error:`, error);
