...
import { 
  ...
  getNotifications, // NOTIFICATIONS API ROUTESimport { db } from '../../../lib/database';

  createNotification, 

  getNotificationSummary,// Phase 7 - SURGICAL MODE

  markAllAsRead,

  markAsRead,// ========================export async function GET(request: NextRequest) {

  hasNotificationAccess

} from '@/services/notifications';  try {
...
export async function GET(request: NextRequest) {
  ...
  try {import {     const recipientId = searchParams.get('recipientId');
    ...
    if (!userCookie) {  createNotification,     const status = searchParams.get('status');

      return NextResponse.json(

        { error: 'Authentication required' },   getNotificationSummary,
  ...
  markAsRead,
    ...
    const userId = userData.id;  hasNotificationAccess    // Filtros
...
// ========================    if (type) {
  ...
  try {      notifications = notifications.filter(n => n.status === status);
    ...
    const result = await getNotifications({
      ...
      userId,    const { searchParams } = new URL(request.url);    }
      ...
      offset    // Get user from headers/cookies (BSOS auth system)    // Ordenar por data de criação (mais recentes primeiro)
    ...
    console.error('GET /api/notifications error:', error);      return NextResponse.json(    return NextResponse.json({ notifications });
