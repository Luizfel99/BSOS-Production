...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const login = async (email: string, password: string, role?: string): Promise<boolean> => {
    ...
    try {
      ...
      // First, try real API login (unless it's explicitly a demo login)
      if (password !== 'demo') {
        try {
          ...
          if (response.ok && data.success) {
            ...
            console.log('🎉 Real API login completed successfully for user:', apiUser.email, 'Role:', apiUser.role);
            return true;
          } else {
            console.log('❌ Real API login failed:', data.error);
            // Fall back to demo login
          }
        } catch (apiError) {
          console.log('⚠️ API login error, falling back to demo:', apiError);
          // Fall back to demo login
        }
      }
      ...
      console.log('✅ Demo login successful, setting user:', {
        id: foundUser.id,
      ...
      console.log('🎉 Demo login completed successfully for user:', foundUser.email, 'Role:', foundUser.role);
      return true;
