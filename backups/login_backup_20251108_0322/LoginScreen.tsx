...
export default function LoginScreen() {
  ...
  useEffect(() => {
    ...
    if (pendingRedirect && user) {
      ...
      console.log('🔍 LoginScreen: User role before switch:', user.role, 'Type:', typeof user.role);
      ...
      switch (user.role) {
        ...
        default: 
          console.log('⚠️ LoginScreen: No case matched, using default /dashboard');
  ...
  const quickLogin = async (userId: string) => {
    ...
    try {
      ...
      // Demo accounts use 'demo123' password, admin uses 'admin123'
      const demoPassword = demoUser.email === 'admin@bsos.com' ? 'admin123' : 'demo123';
      const loginSuccess = await login(demoUser.email, demoPassword);
